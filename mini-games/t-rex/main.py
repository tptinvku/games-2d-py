...
def load_image(name, sizex=-1, sizey=-1, colorkey=None):
    fullname = os.path.join(".\\mini-games\\t-rex\\resource", name)
...
def load_sprite_sheet(sheetname, nx, ny, scalex=-1, scaley=-1, colorkey=None):
    fullname = os.path.join(".\\mini-games\\t-rex\\resource", sheetname)
    print(os.getcwd())
...
def display_gameover_message(replaybutton_image, gameover_image):
    replaybutton_rect = replaybutton_image.get_rect()
    replaybutton_rect.centerx = width/2
    replaybutton_rect.top = height*0.52

    gameover_rect = gameover_image.get_rect()
    gameover_rect.centerx = width/2
    gameover_rect.top = height*0.32
    screen.blit(replaybutton_image, replaybutton_rect)
...
def gameplay():
    ...
    while not gameQuit:
                    ...
                    if event.type == pg.KEYDOWN:
                        if event.key == pg.K_SPACE:
                            if playerDino.rect.bottom == int(0.98*height):
                                ...
                                playerDino.movement[1] = - \
                                    1*playerDino.jumpSpeed
        ...
        while gameOver:
            ...
            highsc.update(high_score)
            if pg.display.get_surface() != None:
                display_gameover_message(retbutton_image, gameover_image)
                if high_score != 0:
                    highsc.draw()
                    screen.blit(HI_image, HI_rect)
                pg.display.update()
